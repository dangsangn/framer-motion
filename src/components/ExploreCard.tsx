...
interface ExploreCardProp extends ExploreWorlds {
  index: number;
  active: string;
...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }: ExploreCardProp) => (
  ...
  >
    ...
    {active !== id ? (
      <h3 className="font-semibold sm:text-[26px] text-[18px] lg:-rotate-[90deg] absolute text-white z-0 lg:bottom-20 lg:origin-[0,0]">
        {title}
      </h3>
    ) : (
